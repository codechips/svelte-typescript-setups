<script lang="ts">
  ...
  const fetchQuote = () => (wisdom = fetch(KANYE_API).then(res => res.json()));
...
<style lang="postcss">
  .preach {
    @apply text-3xl font-bold px-10 py-3 rounded-lg bg-indigo-800 text-indigo-100 outline-none shadow-lg;
  }
  .preach:hover {
    @apply bg-indigo-600;
  }
